...
const TOOLS = [
  ...
  },
  {
    type: 'function',
    function: {
      name: 'consultar_secretario',
      description: 'Consulta al Secretario, un especialista en organizar y planificar el día de Mateo (detecta choques de agenda, huecos de tiempo, sobrecarga, y propone un orden realista). Usala SOLO cuando Mateo pregunte por su día, su agenda, cómo organizarse, si algo choca/se encima, cómo planificar, o qué hacer primero por tiempo/horario. NO la uses para saludos, charla general, ni preguntas triviales — cada consulta tiene un costo. Como mucho una consulta por turno.',
      parameters: {
        type: 'object',
        properties: {
          pregunta: { type: 'string', description: 'La pregunta o pedido concreto para el Secretario, en tus palabras (podés reformular lo que dijo Mateo para que sea más específico)' },
        },
        required: ['pregunta'],
      },
    },
...
Tenés acceso a dos especialistas — ambos son consultores: te devuelven un análisis en texto, nunca ejecutan nada. Si alguno sugiere crear una tarea, mover un bloque o agendar algo, sos VOS quien decide si lo hace (con tus propias herramientas) o si primero se lo proponés a Mateo. Cuando cualquiera de los dos te responda, NUNCA le pegues su texto tal cual a Mateo: reformulalo con tu propio tono, como si la conclusión fuera tuya. Nunca consultes a ninguno para saludos ni charla trivial, y como mucho una consulta a cada uno por turno.

- **Tutor PAES** (consultar_tutor_paes): analiza en profundidad el progreso PAES de Mateo y propone práctica concreta. Usalo cuando la pregunta sea sobre preparación PAES (progreso, debilidades por materia, qué estudiar, práctica).
- **Secretario** (consultar_secretario): organiza y planifica el día de Mateo, detecta choques de agenda, huecos de tiempo y sobrecarga. Usalo cuando la pregunta sea sobre el día, la agenda, cómo organizarse, si algo se encima, o qué hacer primero por tiempo/horario.
